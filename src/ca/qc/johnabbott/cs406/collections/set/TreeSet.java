...
public class TreeSet<T extends Comparable<T> & Serializable> implements Set<T>, Serializable {
    ...
    private void writeTo(Serializer s, Node<T> current) throws IOException{
        // Write current element to serializer
        s.serialize(current.element);

        // Add flag indicating if the node has children and where they are
        // 3 = Two children
        // 2 = Right child only
        // 1 = Left child only
        // 0 = No children
        // After flagging, recursively call on children
        if(current.left != null && current.right != null){
            s.serialize(new SInteger(3));

            writeTo(s, current.left);
            writeTo(s, current.right);
        }
        else if(current.right != null){
            s.serialize(new SInteger(2));
    ...
    private void readFrom(Serializer s, Node<T> current) throws IOException, SerializationException {
        // Get current element from serializer
        current = new Node<>((T) s.deserialize());

        // Get the flag indicating how many children, and where they are
        int childFlag = ((SInteger) s.deserialize()).get();

        // Recursively call on children (if any)
        switch (childFlag){
            case 1: //Left child only
                readFrom(s, current.left);
                break;

            case 2: //Right child only
                readFrom(s, current.right);
                break;
